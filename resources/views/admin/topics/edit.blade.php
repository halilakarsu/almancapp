...
@section('header', 'Konu Düzenle')
@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2 class="h4 fw-bold text-dark mb-0"><i class="bi bi-pencil-square text-primary me-2"></i>Konu Düzenle</h2>
            ...
            <div class="card-body p-5">
                <form action="{{ route('admin.topics.update', $topic) }}" method="POST">
                        ...
                        <select name="lesson_id" class="form-control form-control-lg rounded-3 bg-light border-0 shadow-none @error('lesson_id') is-invalid @enderror" required>
                            @foreach($lessons as $lesson)
                                <option value="{{ $lesson->id }}" {{ $topic->lesson_id == $lesson->id ? 'selected' : '' }}>
                                    {{ $lesson->level?->level_title ?? 'Seviye Yok' }} > {{ $lesson->lesson_title }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="topic_title" id="title_editor" class="form-control @error('topic_title') is-invalid @enderror">{{ old('topic_title', $topic->topic_title) }}</textarea>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="topic_description" id="description_editor" class="form-control @error('topic_description') is-invalid @enderror">{{ old('topic_description', $topic->topic_description) }}</textarea>
